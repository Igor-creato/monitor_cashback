...
## Pre-code gate: актуальные OSS/готовые решения

Перед написанием любого кода, тестов, миграций, API, worker, UI, интеграции или инфраструктурной конфигурации агент обязан оценить, есть ли актуальные, поддерживаемые и безопасные готовые open source решения, библиотеки или сервисы, которые подходят задаче лучше самописной реализации.

Правила выбора:

1. Искать и проверять актуальность по живым источникам на момент работы: official docs, package registry, release notes/changelog, GitHub Releases/tags, security advisories/CVE/OSV/GHSA, endoflife.date для runtimes/services. Для библиотек и фреймворков сначала использовать актуальную документацию через доступные doc-инструменты, если они есть.
2. Если используется готовое OSS-решение, библиотека, Docker image, сервис или action, выбирать последнюю стабильную безопасную версию на момент написания кода. Не использовать beta/rc/nightly/canary, если пользователь явно не попросил и риск не оформлен отдельно.
3. Не добавлять и не предлагать старый, заброшенный, EOL, неподдерживаемый или небезопасный код/пакет/сервис. Красные флаги: нет поддерживаемых релизов, архивный репозиторий, известные unfixed critical/high уязвимости, несовместимая лицензия, отсутствие поддержки текущего runtime, сомнительный supply chain.
4. Если последняя стабильная безопасная версия несовместима с текущим стеком, не фиксировать молча старую версию. Остановиться и предложить варианты: обновить стек, выбрать другой поддерживаемый аналог, отказаться от зависимости или оформить ADR.
5. Перед добавлением зависимости/сервиса зафиксировать в ответе или документации: что выбрано, последняя стабильная версия, откуда проверена актуальность, почему решение подходит, какие риски безопасности/лицензии/поддержки проверены.
6. Если подходящего поддерживаемого OSS-решения нет, явно сказать это и только тогда писать собственную реализацию в минимальном scope с тестами и security review.
